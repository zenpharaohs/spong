...
def _level_crossing(m, kernel, a0, b0, ascend: bool, c: float,
                    ds: float, cap: int):
    """Thin adapter: native GL8 stepping, error control, and event refinement."""
    result = kernel.section_trace(float(a0), float(b0), 1 if ascend else -1,
                                  float(c), float(m.C), float(ds), int(cap))
    if result["status"]:
        raise ArithmeticError(f"native section refusal ({result['status']}): {result['reason']}")
    return np.asarray(result["points"], dtype=float)
