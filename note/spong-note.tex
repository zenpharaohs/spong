...
Both tests are effective over $\Q$: the gcd is computed by the Euclidean
algorithm on rational coefficients, and whether a rational polynomial has a
real root is decided by a Sturm sequence. No floating-point arithmetic and no
tolerance enters, so the verdict is a theorem about the model as given rather
than an estimate.
