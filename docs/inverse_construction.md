...
### Out-of-sample qualification of GL6 (and three bugs it found)

Everything above was IN-sample: the straddling suite is generated by the inverse
construction from three fixed `g` polynomials, and the tricky portrait is one
fixed model.  Two out-of-sample arms, scored on the ledger's own certificates,
GL4 vs GL6 on IDENTICAL models (both arms in C, via the retained
`slow_step_gl4`/`fast_step_gl4`, so the comparison is method-vs-method and not
C-vs-Python):

- **arm 1** — 30 pure random portraits from the house sampler (`cli._coefficients`)
- **arm 2** — 25 TARGETED: random `g` (not the three tuned on) with a critical
  point prescribed at 2^k, k ∈ [3,17] — the stiff regime the recent choices touch

**Three real bugs, none visible in-sample.**

1. **The 3x3 adjugate overflowed 150 orders early.**  It forms TRIPLE products
   where a 2x2 determinant forms double ones: unscaled it dies at |h·J| ~ 1e150
   against GL4's ~1e300.  Out of sample this was a NaN step on a random
   portrait GL4 handled.  Fixed by row-scaling before forming the cofactors —
   free, since the row inf-norms were already computed for the guard, and it
   makes the Hadamard ratio simply |det| of the scaled matrix.  Max worst-E
   2.94e-02 -> 7.79e-04.
2. **`backbone_residual` was mis-scoped.**  It maxed over the WHOLE polyline,
   but the claim "this branch is the backbone" only applies where
   `angle_energy` gave up.  Out of sample it read as 37 of 75 branches
   uncertified when their unresolved tails were in fact fine.  Now scoped to
   the unresolved vertices, returning 0.0 when there is no claim to make:
   max residual 4.57e+14 -> 2.03e-16 (pure), 3.89e+15 -> 1.44e-15 (targeted).
3. **A failed stage solve aborted the branch instead of halving.**  `M = I −
   h·diag(J_i)·A` is diagonally dominant for small enough h, so a guard trip is
   a STEP-SIZE signal, not a fatal error.  GL6's 3-stage solve is tighter than
   GL4's 2-stage one, so it signals where GL4 proceeds silently; without
   halving that read as `abort_nonfinite` on branches GL4 captured.

**Verdict after the fixes.**  Angle energy is a WASH — over 55 portraits, 49 are
ties within 5% and the median ratio is 1.0000 (the raw "GL6 worse 21/30" headline
was almost entirely sub-1% noise).  Seam, the metric GL6 was chosen for, is
clearly better: 14/30 better and 1 worse (pure), 23/25 better and 0 worse
(targeted).  Cost: **one branch in 545** where GL6 aborts and GL4 captures
(253 vs 254 captures); clean portraits 27/30 vs 28/30, with the two others
unclean under both.

**A pre-existing issue this turned up, NOT introduced by GL6.**  The native and
Python paths diverge at large |h·J| — at b = −4.013, h = 1e−2, |h·J| = 2.9e+04:
GL4 gives 4.073 (C) vs 4.721 (Python), GL6 gives −3.60e−02 vs 2.58e−02.  It
affects BOTH methods equally, so it is not the GL6 port.  Note also that these
